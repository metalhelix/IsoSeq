---
title: "IsoSeq Analysis Report - Isofrom Level"
output:
  html_document:
    code_folding: show
params:
...
gene_matrix <- ReadMtx(paste0(seurat_outdir, "/isoforms_seurat/matrix.mtx"),
                  features = paste0(seurat_outdir, "/isoforms_seurat/genes.tsv"),
                  cells = paste0(seurat_outdir, "/isoforms_seurat/barcodes.tsv"),
                  )

saturation_tsv <- params$saturation_file

seurat_df <- CreateSeuratObject(counts = gene_matrix, project="IsoSeq_Isoform", 
...
df_sat <- df_sat[,c("reads","unique_isoforms", "unique_isoforms_known")]
...
seurat_df <- subset(seurat_df, subset = nFeature_RNA > 200 & percent.mt < 5 & percent.ribo < 5 ) # a light filter
```

nFeature_RNA > 200 & percent.mt < 5 & percent.ribo < 5 were used to lightly filter the data. New metrics after data filtering are shown in the table below:
</br>
...
saveRDS(seurat_df, file = "SeuratObj.isoforms.rds")
